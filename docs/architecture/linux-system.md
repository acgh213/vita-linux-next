...
Status: proposed implementation architecture, 2026-09-13. User-selected direction:
**general-purpose Linux with normal package repositories; retain the current
exFAT card/data and prototype using separate ext4 image files.** No migration is
approved by this document alone. Work is sequenced in the [implementation plan](../plans/2026-09-13-linux-system-roadmap.md).
...
workarounds. Alpine armv7 is the bounded fallback comparison, with a separate musl
userland and its own packages. Never mix Debian/Alpine/Buildroot libraries or
package databases into one root.
...
2. **exFAT transport:** keep the current filesystem and existing files untouched.
   Initial trial adds only a separately named Linux image and manifests, after
   backup/headroom/card-health approval. Prefer fully allocated images; do not
   assume sparse allocation behavior on exFAT.
...
No repartitioning in this plan. Native ext4 media could later remove a nested
filesystem failure layer, but it is a different, separately approved migration.
